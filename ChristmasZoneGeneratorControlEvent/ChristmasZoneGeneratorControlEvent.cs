...
namespace TheRiptide
{
    ...
    public class EventHandler
    {
        ...
        private static HashSet<int> scps = new HashSet<int>();
        ...
        {
            string info = "Event being played: " + ChristmasZoneGeneratorControlEvent.Singleton.EventName + "\n<size=24>" + ChristmasZoneGeneratorControlEvent.Singleton.EventDescription + "</size>";
            ChristmasZoneTeamManager.Singleton.AssignTeam(player, chaos, mtf, info);
        }
        ...
        void OnRoundStart()
        {
            ...
            List<int> cmtf = mtf.ToList();
            List<int> cchaos = chaos.ToList();
            for (int i = 0; i < scp_count; i++)
            {
                if (i % 2 == 0)
                    scps.Add(cmtf.PullRandomItem());
                else
                    scps.Add(cchaos.PullRandomItem());
            }
            scp_roles = new List<RoleTypeId>
            {
                RoleTypeId.Scp173,
                RoleTypeId.Scp939,
                RoleTypeId.Scp106,
            ...
            ChristmasZoneTeamManager.Singleton.BroadcastTeamCount(chaos, mtf);
        }
        ...
        bool OnPlayerChangeRole(Player player, PlayerRoleBase oldRole, RoleTypeId new_role, RoleChangeReason reason)
        {
            ...
            if (player == null || !Round.IsRoundStarted ||
                new_role == RoleTypeId.Spectator || new_role == RoleTypeId.Tutorial || new_role == RoleTypeId.Overwatch || new_role == RoleTypeId.Filmmaker || new_role == RoleTypeId.Scp0492)
                return true;

            if (!scp_roles.IsEmpty() && scps.Contains(player.PlayerId) && reason == RoleChangeReason.RoundStart)
            {
                Timing.CallDelayed(0.0f, () =>
                {
                    if (scp_roles.IsEmpty())
                        return;
                    player.ReferenceHub.roleManager.ServerSetRole(scp_roles.PullRandomItem(), RoleChangeReason.RemoteAdmin);
                });
            }
            ...
            else if (chaos.Contains(player.PlayerId))
            {
    ...
    public class ChristmasZoneGeneratorControlEvent : IEvent
    {
        public static ChristmasZoneGeneratorControlEvent Singleton { get; private set; }

        public static bool IsRunning = false;
        public PluginHandler Handler;

        public string EventName { get; } = "Christmas Zone Generator Control";
        ...
        public string EventPrefix { get; } = "CZGC";
        ...
        public void PrepareEvent()
        {
            ...
            IsRunning = true;
            harmony = new Harmony("ChristmasZoneGeneratorControlEvent");
            harmony.PatchAll();
        ...
        {
            IsRunning = false;
            harmony.UnpatchAll("ChristmasZoneGeneratorControlEvent");
            EventHandler.Stop();
            PluginAPI.Events.EventManager.UnregisterEvents<EventHandler>(this);
        }

        [PluginEntryPoint("Christmas Zone Generator Control Event", "1.0.0", "", "The Riptide")]
