...
namespace TheRiptide
{
    public class Bracket
    {
        ...
        //https://discord.com/channels/974269642480685056/1039316650371068066/1157429740647489546
        public string GetCurrentStandingHint()
        {
            const int line_width = 130;
            ...
            int total_width = stage_width * (stages + 1);

            //string background = "<mspace=1.0em><mark=#000000FE><line-height=100%>";
            //for (int l = min_line; l <= max_line; l += 2)
            //    background += "|" + new string(' ', Mathf.Max((stage_width * (stages + 1)) / 2 - 2, 0)) + "|\n";
            //background += "</mark>";
        ...
        private string ToLatinChars(string str)
        {
            ...
            List<char> arr = str.ToList();
            for(int i = 0; i < arr.Count; i++)
                if (replacements.ContainsKey(arr[i]))
        ...
        private void BuildStageLines(Node node, int line, int stage, bool is_a, int max_char_width, Dictionary<int, Dictionary<int, string>> line_stages)
        {
            ...
            if(node.winner == null)
            {
                segment = prefix + ToLatinChars("[" + new string('_', max_char_width - 4) + "] ");
                if(node != root)
                {
            ...
            {
                segment = node.winner.BadgeColor + prefix + ToLatinChars(node.winner.BadgeName.PadLeft(max_char_width - 2, pad_name).Substring(0, max_char_width - 2)) + sufix + "</color>";
                if (node != root)
                {
                    for (int i = 1; i < (1 << stage); i++)
                    {
