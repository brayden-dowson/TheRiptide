...
namespace TheRiptide
{
    ...
    class ArenaManager
    {
        ...
        public static void Reset(Zone zone)
        {
            ...
            foreach(var door in DoorVariant.AllDoors)
            {
                if (door.Rooms.IsEmpty() || door.Rooms.First().Zone != facility_zone || (door.Rooms.Count() == 1 && door.Rooms.First().Name == RoomName.Lcz914))
                    continue;
            ...
            if(facility_zone == FacilityZone.LightContainment)
            {
                RoomIdentifier scp330room = RoomIdentifier.AllRoomIdentifiers.FirstOrDefault(r => r.Name == RoomName.Lcz330);
                if(scp330room != null)
                {
                    Scp330Interobject scp330 = scp330room.GetComponentInChildren<Scp330Interobject>();
                    scp330._takenCandies.Clear();
                }
            }
        }
