    ...
    public class Match
    {
        ...
        private IEnumerator<float> _BanSelection()
        {
            ...
            while (elapsed < EventHandler.config.BanSelectionTime && (!team_a_ready || !team_b_ready))
            {
                try
                {
                    team_a_ready = RunBanLogic(rooms.team_a, team_a_ready_players, team_a, EventHandler.config.BanSelectionTime - elapsed);
                    team_b_ready = RunBanLogic(rooms.team_b, team_b_ready_players, team_b, EventHandler.config.BanSelectionTime - elapsed);
                }
        ...
        private bool RunBanLogic(LoadoutRoom room, HashSet<int> ready_players, Team team, int time_left)
        {
        ...
        private IEnumerator<float> _LoadoutSelection()
        {
            ...
            while (elapsed < EventHandler.config.LoadoutSelectionTime && (!team_a_ready || !team_b_ready))
            {
                try
                {
                    team_a_ready = RunLoadoutLogic(rooms.team_a, team_a_ready_players, team_a, EventHandler.config.LoadoutSelectionTime - elapsed);
                    team_b_ready = RunLoadoutLogic(rooms.team_b, team_b_ready_players, team_b, EventHandler.config.LoadoutSelectionTime - elapsed);
                }
                catch (System.Exception ex)
                {
                    PluginAPI.Core.Log.Error(ex.ToString());
        ...
        private bool RunLoadoutLogic(LoadoutRoom room, HashSet<int> ready_players, Team team, int time_left)
        {
            bool all_ready = !ready_players.IsEmpty();
            foreach (var user in team.Users)
            {
